...
author: "Alejandro Hernández Beneito"
...
```{r}
mammals_1<-row.names(mammals)
Animals2_1<-row.names(Animals2)
commonAnimals<-intersect(mammals_1, Animals2_1)
print(commonAnimals)
```
...
```{r}
mammals_2<-setdiff(mammals_1, commonAnimals)

print(mammals_2)# En el conjunto mammals hay 4 animales que no están en el otro conjunto.

Animals2_2<-setdiff(Animals2_1, commonAnimals)

print(Animals2_2)# En el conjunto Animals2 hay 7 animales que no están en el otro conjunto.
...
```{r}
setdiff(mammals_2, Animals2_2)
setdiff(Animals2_2, mammals_2)
```
...
#Presentan un comportamineto parecido pero no igual, podemos aseguar al 95% que se tratan de distribuciones gaussianas
```
...
#c)
summary(library())
#Hay un total de 645 ibrerias instaladas
    ```
...
#No hay valores perdidos por lo que el número de valores perdidos es 
...
#Si que verifican esta condición
...
#Existen diferencias en los histogramas, por lo que existen diferencias en las vitaminas.
```
...
#No parece que exista una recta que se pueda adaptar tan bien a este modelo como en el anterior
...
abline(Model_c39, lty=3)

abline(Model_c52)



#Los puntos negros representan a c39 y los blancos a c52
